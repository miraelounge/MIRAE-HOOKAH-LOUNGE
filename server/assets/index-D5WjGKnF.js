...
import { u as useConsent } from "./router-v2tCmHx4.js";
...
function Hero() {
  const lounge1Url = lounge1.url.startsWith("/") ? `${"/MIRAE-HOOKAH-LOUNGE/".replace(/\/$/, "")}${lounge1.url}` : lounge1.url;
...
function Atmosphere() {
  ...
  const lounge1Url = lounge1.url.startsWith("/") ? `${"/MIRAE-HOOKAH-LOUNGE/".replace(/\/$/, "")}${lounge1.url}` : lounge1.url;
  const lounge2Url = lounge2.url.startsWith("/") ? `${"/MIRAE-HOOKAH-LOUNGE/".replace(/\/$/, "")}${lounge2.url}` : lounge2.url;
...
function Gallery() {
  ...
  const getImageUrl = (id) => {
    const basePath = "/MIRAE-HOOKAH-LOUNGE/".replace(/\/$/, "");
